...
namespace yangyongxu\wechat\src;


use yangyongxu\wechat\base\WechatBase;
...
class Extend extends WechatBase
{
    ...
    public function getQRCode($scene_id, $type = 0, $expire = 2592000)
    {
        ...
        if ($type && !is_numeric($scene_id)){
        	$type_text = 'QR_LIMIT_STR_SCENE';
        }elseif ($type){
        	$type_text = 'QR_LIMIT_SCENE';
        }elseif (!is_numeric($scene_id)){
        	$type_text = 'QR_STR_SCENE';
        }else{
        	$type_text = 'QR_SCENE';
        }
        $data = [
            ...
            'action_info'    => ['scene' => (!is_numeric($scene_id)?['scene_str' => $scene_id] : ['scene_id' => $scene_id])]
        ];
